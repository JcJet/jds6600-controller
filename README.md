...
- **Windows 7 x64**: есть отдельная legacy-сборка на Python 3.8 для старых систем.
...
1. Скачайте архив для своей ОС (`JDS6600Controller-windows.zip`, `JDS6600Controller-windows7-legacy.zip` или `JDS6600Controller-linux.zip`) и распакуйте его.
2. Подключите генератор по USB.
3. Запустите исполняемый файл из распакованной папки.
4. Папка `programs/` рядом с программой предназначена для ваших файлов команд — диалог «Открыть» по умолчанию открывается в ней.
5. Нажмите **Авто‑поиск** (или выберите порт вручную) — в статусе должно стать **«устройство найдено»**.
6. Нажмите **Старт**.
...
- Используйте архив `JDS6600Controller-windows7-legacy.zip`.
- Эта сборка предназначена для старых систем и собирается на Python 3.8.
- На целевой системе Windows 7 должно быть установлено обновление **KB2533623**.
- Для более новых Windows используйте обычный архив `JDS6600Controller-windows.zip`.
