...
- Добавлена сборка **Windows 7 x64 legacy**.
- Добавлены опции автовыключения каналов и выключения ПК по завершении скрипта.
- Добавлена опция автовключения каналов при старте скрипта, по умолчанию включено. Сделана для удобства использования при автовыключении - чтобы не приходилось вручнуюу включать каналы.
...
- Workflow теперь собирает готовые release-архивы для обеих платформ:
  - `JDS6600Controller-linux.zip` — AppImage + папка `programs/`
  - `JDS6600Controller-windows.zip` — `JDS6600Controller-win32.exe` + `JDS6600Controller-win64.exe` + папка `programs/`
- В репозиторий добавлена папка `programs/` (можно хранить пользовательские сценарии рядом с приложением).

#### GUI / открытие файлов
- Диалог **«Открыть файл команд»** и **«Сохранить как»** по умолчанию предлагают папку `programs/` (или папку текущего файла, если файл уже открыт).
- На первом запуске без сохранённых настроек стартовый пример ищется сначала в `programs/`, затем в корне проекта.

#### Подключение
- Исправлена стартовая гонка между **проверкой порта** и **подключением**: при первом запуске/автопоиске программа больше не должна показывать ложную ошибку подключения перед успешным коннектом.
- Для GUI-проверки порта и ручного подключения добавлена сериализация доступа к порту, чтобы probe/connect не мешали друг другу.
