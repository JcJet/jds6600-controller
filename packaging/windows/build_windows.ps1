...
& "$VenvDir\Scripts\pip.exe" install -U pip wheel setuptools
& "$VenvDir\Scripts\pip.exe" install -r requirements.txt
# Pillow helps PyInstaller convert PNG icon to ICO automatically
& "$VenvDir\Scripts\pip.exe" install pyinstaller pillow

# Build EXE (GUI, no console)
& "$VenvDir\Scripts\pyinstaller.exe" packaging\windows\JDS6600Controller.spec --clean --noconfirm
